...
class _AvatarImageState extends State<AvatarImage> {
  ...
  Future uploadPic(BuildContext context) async {
    ...
    StorageReference firebaseStorageRef = FirebaseStorage.instance.ref().child(globals.userData.uid).child(fileName);
    StorageUploadTask uploadTask = firebaseStorageRef.putFile(_image);
    StorageTaskSnapshot taskSnapshot = await uploadTask.onComplete;

    //Retrieve the image link from FirebaseStorage
    String downloadUrl = await taskSnapshot.ref.getDownloadURL();
    await DatabaseService(uid: globals.userData.uid).updateUserData(globals.userData.fullName!, globals.userData.email!, avatar: downloadUrl);
    ...
    return Container(
      ...
      child: Stack(
        children: <Widget>[
          ...
          CircleAvatar(
            radius: 10.0 * 5,
            child: loading ? Loading() : _imgUrl == '' 
              ? Icon(Icons.person, color: Colors.white, size: 10.0 * 5,)
              :ClipOval(
          ...
          Align( alignment: Alignment.bottomRight,
            child: Container(
              height: 10.0 * 2.5,
              width: 10.0 * 2.5,
              decoration: BoxDecoration(color: Color(0xFFb333fa), shape: BoxShape.circle),
              child: Center(
                ...
                child: GestureDetector(
                  ...
                  child: Icon(Icons.edit, size: 15.0, color: Colors.white,),
                )
              ),
            ),
          ),
